  ...
  it("matches relayer-relevant revert conditions and succeeds for valid input", async () => {
    ...
    const deterministic = await preflightRunMacro({
      chain: {
        ...
        rpcs: [{ name: "local", url: rpcUrl }],
        confirmations: 1,
        superfluidHost: "0x0000000000000000000000000000000000000009",
        forwarders: { clearMacroV1: forwarder, permit2ClearMacroV1: forwarder },
        providers: ["macros.superfluid.eth"],
        macros: [{ address: macro, name: "Macro", enabled: true, supportedKinds: ["clearMacroV1"] }],
      },
      forwarder,
      macro,
      params,
